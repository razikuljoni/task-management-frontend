...
const AddTask = () => {
    ...
    const handleCompleteTask = (id) => {
        Swal.fire({
            title: "Are you sure?",
            text: "You want to make complete this task?",
            icon: "warning",
            ...
            if (result.isConfirmed) {
                fetch(
                    `http://localhost:8000/users_data/${id}?userId=${data[0]._id}&status=complete`,
                    {
                        method: "PATCH",
                        headers: { "Content-Type": "application/json" },
                    }
    ...
    const handleEditTask = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(
                    `http://localhost:8000/users_data/${id}?userId=${data[0]._id}`
                )
                    .then((res) => res.json())
